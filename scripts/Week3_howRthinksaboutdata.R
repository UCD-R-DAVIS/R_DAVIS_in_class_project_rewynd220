...
##Data frames ----
#one of the most common data types; picky lists; can have multiple types of variables, but they all have to be same length 
letters
...
###Arrays ----
#are matrices in 3 dimensions with x,y and z values 
...
#factors in class 
animals<-factor(c("pigs","duck","duck","goose","goose")) #alphabetizes 
animals
levels(animals)
nlevels(animals)

#re-ordering factor so R will plot it how we want it to 
animals<-factor(x=animals,levels = c("goose","pigs","duck")) #tab to pull up options for function 
animals
...
year
class(year)
as.numeric(year) #converts factor to numeric 
#factor is combination of some string value and order of string - if you convert it 

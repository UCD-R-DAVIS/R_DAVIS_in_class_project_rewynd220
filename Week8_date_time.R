...
#non tidyverse way ----
#nicely formatted date and time to format info in a clear way 
#first we'll learn non tidyverse way, then learn tidyverse way 
...
sample_dates_1 <- c("2018-02-01", "2018-03-21", "2018-10-05", 
                    "2019-01-01", "2019-02-18") #helpful to start with character string rather than numbers 
...
#now we tell R to format it as date object 
...
#otherwise this happens (if we have month day year order)
...
sample_dates_3 #did not transfer correctly because not in year date month format 
...
sample_dates_3<- as.Date(sample_dates_2, format = "%m-%d-%Y" )  #specifying the format that date should be read in 
# date code preceded by "%"

sample_dates_3 #now it worked 


#here is a complete list of date-time formats 
?strptime #shows all the codes that you might have a date in and how to specify R to translate 
...
#sometimes incorrect number of digits 
...
#lubridate can handle slashes instead of hyphens 
...
#use these letters before instead of all the %s 
...
mloa2 <- read_csv("https://raw.githubusercontent.com/UCD-R-DAVIS/R-DAVIS/main/data/mauna_loa_met_2001_minute.csv",
                  col_types = "cccccccddddddddd") #each letter specifies the type of data class you want it to import 
...
#you could also deal with month columns individually instead of entering the little lettters when you read something in 
#how to change individual column 

mloa1$day <- as.character(mloa1$day )
...
mloa1$datetime <- ymd_hm(as.character(mloa2$datetime), 
                         tz="Pacific/Honolulu")
tz(mloa1$datetime_test2) #tz is a way to get back the timezone 
...
months <- month(mloa2$datetime_test2)
...
#make sure to use the correct time zone or else errors could occur 

#make a sequence of dates using seq 

#in lubridate function called decimal_date (looks weird but helps order things correctly or can subtract more easily)
